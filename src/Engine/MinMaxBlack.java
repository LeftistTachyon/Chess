 ...
 * Min-Max search function that searches through
 * the chess game tree in Black's perspective.
 * Larger values indicate better positions for Black and vice versa.
 * @author Will
 */
final class MinMaxBlack {
    ...
    /**
     * Minimizing component of the Min-Max search function seeking
     * to reduce Black's score as much as possible. This component implements
     * White's moves.
     *
     ...
     * possible. This score may be extremely high, indicating that White is losing
     * or is about to be checkmated.
     */
    static int min(final Grid grid, final List<Piece> whites, final List<Piece> blacks, int depth) {
...

        {
            ...
            for (int index = (castleTiles.size() - 1); index >= 0; --index) {
                ...
                if (kingCastleTile.getColumn() == ChessConstants.LEFT_KING_CASTLE_COLUMN) {
                    ...
                    int result = max(grid, whites, blacks, depth);
                    if (result < min) {
                        min = result;
                    }
    ...
    /**
     * Maximizing component of the Min-Max search function. This component
     ...
     * possible. This score may be extremely low, indicating that Black is losing
     * or is about to be checkmated.
     */
    static int max(final Grid grid, final List<Piece> whites, final List<Piece> blacks, int depth) {
        if (depth == 0 || AI.TIMER.timeOver()) {
            return Evaluator.evaluateInBlackPerspective(grid, whites, blacks);
        }
...

        {
            ...
            for (int index = (castleTiles.size() - 1); index >= 0; --index) {
                ...
                if (kingCastleTile.getColumn() == ChessConstants.LEFT_KING_CASTLE_COLUMN) {
                    ...
                    leftRook.increaseMoveCount();
                    int result = min(grid, whites, blacks, depth);
                    if (result > max) {
                        max = result;
                    }
