...
public final class Grid implements Renderable {
    ...
    private final Tile[] tiles = new Tile[ChessConstants.LINEAR_LENGTH];
    
    public Grid(int x, int y, final int width, final int height) {
        final int originalX = x;
        for (int index = 0, row = 0, column = 0; index < ChessConstants.LINEAR_LENGTH; ++index) {
    ...
    public Tile getTile(int row, int column) {
        return tiles[ChessConstants.LENGTH * row + column];
    }
    ...
    public List<Piece> getPieces() {
        List<Piece> pieces = new ArrayList<>(ChessConstants.MAX_NUMBER_OF_PIECES);
